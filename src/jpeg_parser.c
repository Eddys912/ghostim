...
static unsigned int tiff_type_size(unsigned short type) {
    switch (type) {
        case TIFF_TYPE_BYTE:  case TIFF_TYPE_ASCII: return 1;
...
static void tiff_read_ascii(const TiffCtx *ctx, unsigned int offset,
    ...
    if (offset + n > ctx->size) { buf[0] = '\0'; return; }
...
static void walk_gps_ifd(const TiffCtx *ctx, unsigned int ifd_offset,
    ...
    for (unsigned short i = 0; i < entry_count; i++, entry_offset += 12) {
        if (entry_offset + 12 > ctx->size) break;
...
static void walk_ifd0(const TiffCtx *ctx, unsigned int ifd_offset,
    ...
    for (unsigned short i = 0; i < entry_count; i++, entry_offset += 12) {
        ...
        case TAG_MODEL:
            ...
            break;
        case TAG_DATETIME:
            tiff_read_ascii(ctx, voff, count, info->datetime,
                            sizeof(info->datetime));
            break;
        case TAG_SOFTWARE:
            tiff_read_ascii(ctx, voff, count, info->software,
                            sizeof(info->software));
            break;
        case TAG_IMAGE_WIDTH:
            info->width = (type == TIFF_TYPE_SHORT)
                          ? tiff_u16(ctx, voff) : tiff_u32(ctx, voff);
            break;
        case TAG_IMAGE_HEIGHT:
            info->height = (type == TIFF_TYPE_SHORT)
                           ? tiff_u16(ctx, voff) : tiff_u32(ctx, voff);
            break;
        case TAG_GPS_IFD:
...
static int parse_app1_exif(const unsigned char *seg_data, size_t seg_len,
    ...
    if (memcmp(seg_data, "Exif\0\0", 6) != 0) return -1;

    TiffCtx ctx;
    ctx.data = (unsigned char *)seg_data + 6;
    ctx.size = seg_len - 6;

    if (ctx.size < 8) return -1;
...
int jpeg_print_info(const char *path, int verbose) {
    ...
    while (pos + 3 < file_size) {
        ...
        if (marker == MARKER_SOI) { pos += 2; continue; }
    ...
    if (info.make[0])     printf("|   Make     : %-30s|\n", info.make);
    if (info.model[0])    printf("|   Model    : %-30s|\n", info.model);
    if (info.datetime[0]) printf("|   DateTime : %-30s|\n", info.datetime);
    if (info.software[0]) printf("|   Software : %-30s|\n", info.software);
...
static int segment_should_drop(unsigned char marker) {
    if (marker == MARKER_APP1) return 1;              /* EXIF / XMP       */
    if (marker == MARKER_APP0) return 1;              /* JFIF header      */
    if (marker == MARKER_COM)  return 1;              /* Text comments    */
    if (marker >= 0xE3 && marker <= 0xEF) return 1;  /* APP3..APPF       */
    return 0;
}
...
static void strip_gps_from_app1(unsigned char *seg_data, size_t seg_len) {
    if (seg_len < 6 || memcmp(seg_data, "Exif\0\0", 6) != 0) return;

    TiffCtx ctx;
    ctx.data = seg_data + 6;
    ctx.size = seg_len - 6;
    if (ctx.size < 8) return;

    ctx.little_endian = (ctx.data[0] == 'I');

    unsigned int ifd0_offset   = tiff_u32(&ctx, 4);
    if (ifd0_offset + 2 > ctx.size) return;
    ...
    for (unsigned short i = 0; i < entry_count; i++, entry_off += 12) {
        if (entry_off + 12 > ctx.size) break;
...
#define APPEND(ptr, n) do {                                     \
    ...
    while (pos + 3 < file_size) {
        ...
        if (marker == MARKER_SOI) { APPEND(buf + pos, 2); pos += 2; continue; }
        if (marker == MARKER_EOI) { APPEND(buf + pos, 2); break; }
        /* Skip pad bytes (0xFF stuffing) */
        if (marker == 0xFF)       { pos++; continue; }

        if (pos + 3 >= file_size) break;
        unsigned short seg_len = read_be16(buf + pos + 2);
        if (seg_len < 2) break;
        ...
        if (drop) {
            ...
            if (marker == MARKER_APP1 && strip_mode == STRIP_GPS) {
                ...
                if (verbose) printf("  [patch]  GPS removed from APP1\n");
            } else {
