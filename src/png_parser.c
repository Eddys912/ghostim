...
int png_print_info(const char *path, int verbose) {
  ...
  while (pos + 12 <= file_size) {
    ...
    if (chunk_is(type_ptr, "tEXt") || chunk_is(type_ptr, "iTXt")) {
      ...
      size_t kw_len = 0;
      while (kw_len < chunk_len && data_ptr[kw_len] != '\0')
        kw_len++;
      char keyword[80];
      ...
      printf("| eXIf chunk found (%u bytes)%*s|\n", chunk_len,
             (int)(19 - snprintf(NULL, 0, "%u bytes", chunk_len)), " ");
      ...
      printf("| Last modified : %04u-%02u-%02u %02u:%02u:%02u      |\n", year,
             data_ptr[2], data_ptr[3], data_ptr[4], data_ptr[5], data_ptr[6]);
      found_meta = 1;
